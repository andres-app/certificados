...
class Usuario extends Conectar
{
    ...
    public function get_certificados_todos_usuarios()
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT 
                    ...
                    tm_curso.cur_fechini,
                    tm_curso.cur_fechfin,
                    tm_instructor.inst_nom,    -- Agregar el nombre del instructor
                    tm_instructor.inst_apep    -- Agregar el apellido paterno del instructor
                ...
                INNER JOIN tm_instructor ON tm_curso.inst_id = tm_instructor.inst_id  -- Hacer JOIN con la tabla del instructor
                WHERE td_curso_usuario.est = 1
                ORDER BY td_curso_usuario.curd_id DESC
                LIMIT 10";
        $sql = $conectar->prepare($sql);
        $sql->execute();
        return $sql->fetchAll();
