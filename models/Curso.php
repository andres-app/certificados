<?php
    class Curso extends Conectar{
        ...
        public function insert_curso($cat_id, $cur_nom, $cur_descrip, $cur_fechini, $cur_fechfin, $inst_id, $total_horas){
            $conectar = parent::conexion();
            parent::set_names();
            $sql = "INSERT INTO tm_curso (cur_id, cat_id, cur_nom, cur_descrip, cur_fechini, cur_fechfin, inst_id, cur_img, fech_crea, est, total_horas) VALUES (NULL, ?, ?, ?, ?, ?, ?, '../../public/465730749.png', now(), '1', ?);";
            ...
            $sql->bindValue(7, $total_horas);
            $sql->execute();
            return $resultado = $sql->fetchAll();
        }        

        public function update_curso($cur_id, $cat_id, $cur_nom, $cur_descrip, $cur_fechini, $cur_fechfin, $inst_id, $total_horas){
            $conectar = parent::conexion();
            parent::set_names();
            $sql = "UPDATE tm_curso
                    SET cat_id = ?, cur_nom = ?, cur_descrip = ?, cur_fechini = ?, cur_fechfin = ?, inst_id = ?, total_horas = ?
                    WHERE cur_id = ?";
            $sql = $conectar->prepare($sql);
            $sql->bindValue(1, $cat_id);
            $sql->bindValue(2, $cur_nom);
        ...
        public function get_curso(){
            ...
            $sql="SELECT
                ...
                tm_curso.cat_id,
                tm_curso.cur_img,
                tm_curso.total_horas,  -- Agrega este campo
        ...
        public function get_curso_id($cur_id){
            $conectar = parent::conexion();
            parent::set_names();
            $sql = "SELECT cur_id, cat_id, cur_nom, cur_descrip, cur_fechini, cur_fechfin, inst_id, total_horas 
                    FROM tm_curso 
                    WHERE est = 1 AND cur_id = ?";
            $sql = $conectar->prepare($sql);
            $sql->bindValue(1, $cur_id);
            $sql->execute();
            return $resultado = $sql->fetchAll();
